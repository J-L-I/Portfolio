...
function ProjectItem(project: {title: string, imgUrl: string, stack: string[], linkUrl: string}): JSX.Element {
    return (
        <div className="rounded-md border-2 border-blue-900 overflow-hidden">
            <a href={project.linkUrl} target="_blank" rel="noreferrer noopener"><img src={project.imgUrl} alt="Project" className="w-full h-36 md:h-48 object-cover cursor-pointer"/></a>
            <div className="w-full p-4">
                ...
                <p className="flex flex-wrap gap-2 flex-row items-center justify-start text-sm md:text-sm">
                    {project.stack.map(item => (
                        <span className="text-white inline-block px-2 py-1 font-semibold border-2 border-blue-900 rounded-md">
                            {item}
                        </span>
                    ))}
                </p>
